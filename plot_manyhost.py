...
for i in range(H):
    sns.lineplot(data=dfK, x='Time', y='H'+str(i+1), hue='Par', alpha=0.1, ax=a[0], palette=['b','r'])
a[0].legend(lines, labels)
a[0].set_xlabel('Time')
a[0].set_ylabel('Fitness value')
a[0].set_title('Evolution of mean K value in 20 replicates\n K1 = 5000, K2 = 1000, K1max = inf, K2max = inf \n K10/K20 = 0.25, m = 100, b = 0.05, T = 500')
...
labels = ['Microbe 1', 'Microbe 2']
for i in range(H):
    sns.lineplot(data=dfI, x='Time', y='H'+str(i+1), hue='Par', alpha=0.1, ax=a[1], palette=['b','r'])
a[1].legend(lines, labels)
a[1].set_xlabel('Time')
a[1].set_ylabel('Alpha value')
a[1].set_title('Evolution of mean Alpha values in 20 replicates')
...
labels = ['Microbe 1', 'Microbe 2']
for i in range(H):
    sns.lineplot(data=dfM, x='Time', y='H'+str(i+1), hue='Par', alpha=0.1, ax=a[2], palette=['b','r'])
a[2].legend(lines, labels)
a[2].set_xlabel('Time')
a[2].set_ylabel('Population size')
a[2].set_title('Evolution of Population sizes in 20 replicates')
